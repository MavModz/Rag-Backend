...
a JSON list or an uploaded CSV. Idempotent by admin email — re-running skips rows
whose admin already exists. Each row is committed independently so one bad row
...
_CSV_FIELDS = [
    "name", "slug", "plan", "admin_email", "admin_password",
...
def parse_csv(content: bytes) -> list[schemas.ProvisionRow]:
    ...
    for raw in reader:
        ...
        rows.append(schemas.ProvisionRow(**{k: clean.get(k) for k in _CSV_FIELDS if k in clean}))
    return rows
...
async def provision_one(session: AsyncSession, row: schemas.ProvisionRow) -> schemas.ProvisionResult:
    # Idempotency: skip if the admin user already exists.
    if await identity_repo.get_user_by_email_any_tenant(session, row.admin_email) is not None:
    ...
    await identity_service.create_user(
        session, tenant_id=tenant.id, email=row.admin_email, password=password,
        name=row.name, phone=None, role_names=["admin"],
    )
    ...
    if row.ds_type and row.ds_conn:
        ds = await ds_service.create(
            ...
            ds_schemas.DataSourceCreate(
                type=row.ds_type, name="primary",
                config=_build_ds_config(row), field_mapping=row.ds_field_mapping or {}, enabled=True,
            ),
        )
        data_source_id = str(ds.id)

    return schemas.ProvisionResult(
        name=row.name, status="created", tenant_id=str(tenant.id), admin_email=row.admin_email,
        admin_password=password if generated else None, api_key=full_key, data_source_id=data_source_id,
    )
...
async def provision_bulk(
    ...
    for row in rows:
        try:
            ...
            await session.commit()  # commit each row independently
